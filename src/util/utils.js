 ...
 * esc监听全屏
 */
export const fullscreenListener = (callback) => {
  function listen () {
    callback()
  }
...
export const findArray = (dic, value) => {
  return dic.findIndex((val) => val === value)
}
...
export const openWindow = (url, title, w, h) => {
  ...
  const newWindow = window.open(url, title, 'toolbar=no, location=no, directories=no, status=no, ' +
